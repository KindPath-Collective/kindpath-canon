...
| `THE_FIELD_EQUATION.md` | Active | **Foundational.** Φ = km^n — the master equation underlying all KindPath field descriptions. Extractive systems assume n=1. Living systems are n=2. The forgazi is the exponent gap. Instances: F=ma (n=1, navigation), F=am² and E=(MC)² (n=2, generative/recognition), Metcalfe's Law (n=2, network), Reed's Law (n>2, emergent). |
